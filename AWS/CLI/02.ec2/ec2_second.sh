...
PUBLIC_SUBNET_ID=$(aws ec2 describe-subnets --filters Name=tag:Name,Values=${PREFIX}-public-subnet-1c --query "Subnets[*].SubnetId" --output text) && echo $PUBLIC_SUBNET_ID
...
# Get the lastest AMI
aws ec2 describe-images --owner self --filters Name=tag:Name,Values=${PREFIX}-web-01_* --query "sort_by(Images, &CreationDate)[-1].ImageId" --output text
